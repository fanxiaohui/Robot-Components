...
void writeReg(u8 reg, u8 value)
{
	...
	Wire_write(reg);
	Wire_write(value);
	last_status = Wire_endTransmission();
}
...
void writeReg16Bit(u8 reg, u16 value)
{
	Wire_beginTransmission(address);
	Wire_write(reg);
	Wire_write((value >> 8) & 0xFF); // value high byte
	Wire_write( value       & 0xFF); // value low byte
	last_status = Wire_endTransmission();
}
...
void writeReg32Bit(u8 reg, u32 value)
{
	Wire_beginTransmission(address);
	Wire_write(reg);
	Wire_write((value >> 24) & 0xFF); // value highest byte
	Wire_write((value >> 16) & 0xFF);
	Wire_write((value >>  8) & 0xFF);
	Wire_write( value        & 0xFF); // value lowest byte
	last_status = Wire_endTransmission();
}
...
u16 readReg16Bit(u8 reg)
{
	...
	Wire_requestFrom(address, (u8)2);
	value  = (u16)Wire_read() << 8; // value high byte
	value |=           Wire_read();      // value low byte
...
u32 readReg32Bit(u8 reg)
{
	...
	Wire_requestFrom(address, (u8)4);
	value  = (u32)Wire_read() << 24; // value highest byte
	value |= (u32)Wire_read() << 16;
	value |= (u16)Wire_read() <<  8;
	value |=           Wire_read();       // value lowest byte
...
void readMulti(u8 reg, u8 * dst, u8 count)
{
	...
	Wire_requestFrom(address, count);
