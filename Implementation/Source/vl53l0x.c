...
u8 last_status; // status of last I2C transmission

u8 address = ADDRESS_DEFAULT;
u16 io_timeout = 0;
...
{
	writeReg(VL53L0X_I2C_SLAVE_DEVICE_ADDRESS, new_addr & 0x7F);
	address = new_addr;
}
...
// mode.
bool internal_init(bool io_2v8)
{
	...
	if (io_2v8)
	{
		writeReg(VL53L0X_VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV,
		readReg(VL53L0X_VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV) | 0x01); // set bit 0
	}
...
{
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_transmit(ADDRESS_DEFAULT, &value, 1);
}
...
void writeReg16Bit(u8 reg, u16 value)
{
	...
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_transmit(ADDRESS_DEFAULT, valueArray, 2);
}
...
{
	u8 valueArray[4];
	valueArray[0] = (value >> 24) && 0xff;
	valueArray[1] = (value >> 16) && 0xff;
	valueArray[2] = (value >> 8) && 0xff;
	valueArray[3] = value & 0xff;
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_transmit(ADDRESS_DEFAULT, valueArray, 4);
}
...
u8 readReg(u8 reg)
{
	...
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_receive(ADDRESS_DEFAULT, &value, 1);
...
u16 readReg16Bit(u8 reg)
{
	...
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_receive(ADDRESS_DEFAULT, valueArray, 2);
	value |= (u16) valueArray[0] << 8;
	value |= valueArray[1];
...
u32 readReg32Bit(u8 reg)
{
	...
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_receive(ADDRESS_DEFAULT, valueArray, 4);
	value |= (u32) valueArray[0] << 24;
	value |= (u32) valueArray[1] << 16;
	value |= (u32) valueArray[2] << 8;
	value |= valueArray[3];
...
{
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_transmit(ADDRESS_DEFAULT, (u8 *)src, count);
}
...
{
	i2c_transmit(ADDRESS_DEFAULT, &reg, 1);
	i2c_receive(ADDRESS_DEFAULT, dst, count);
}
...
bool setMeasurementTimingBudget(u32 budget_us)
{
	...
	if (enables.final_range)
	{
		...
		writeReg16Bit(VL53L0X_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI,
...
bool setVcselPulsePeriod(vcselPeriodType type, u8 period_pclks)
{
	...
	if (type == VcselPeriodPreRange)
	{
		...
		writeReg16Bit(VL53L0X_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI,
		...
		writeReg(VL53L0X_MSRC_CONFIG_TIMEOUT_MACROP,
	...
	u8 sequence_config = readReg(VL53L0X_SYSTEM_SEQUENCE_CONFIG);
	writeReg(VL53L0X_SYSTEM_SEQUENCE_CONFIG, 0x02);
...
void startContinuous(u32 period_ms)
{
	...
	if (period_ms != 0)
	{
		...
		u16 osc_calibrate_val = readReg16Bit(VL53L0X_OSC_CALIBRATE_VAL);
		...
		writeReg(VL53L0X_SYSRANGE_START, 0x04); // VL53L0X_REG_SYSRANGE_MODE_TIMED
	}
	else
	{
		// continuous back-to-back mode
		writeReg(VL53L0X_SYSRANGE_START, 0x02); // VL53L0X_REG_SYSRANGE_MODE_BACKTOBACK
	}
}
...
{
	writeReg(VL53L0X_SYSRANGE_START, 0x01); // VL53L0X_REG_SYSRANGE_MODE_SINGLESHOT
...
u16 readRangeContinuousMillimeters(void)
{
	...
	u16 range = readReg16Bit(VL53L0X_RESULT_RANGE_STATUS + 10);
...
u16 readRangeSingleMillimeters(void)
{
	...
	startTimeout();
	while (readReg(VL53L0X_SYSRANGE_START) & 0x01)
	{
		if (checkTimeoutExpired())
		{
			did_timeout = TRUE;
...
void getSequenceStepTimeouts(SequenceStepEnables_t const * enables, SequenceStepTimeouts_t * timeouts)
{
	...
	decodeTimeout(readReg16Bit(VL53L0X_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI));
	...
	decodeTimeout(readReg16Bit(VL53L0X_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI));
...
{
	writeReg(VL53L0X_SYSRANGE_START, 0x01 | vhv_init_byte); // VL53L0X_REG_SYSRANGE_MODE_START_STOP
...
u16 vl53l0x_getDistance()
{
	...
	u16_returnValue = readRangeContinuousMillimeters();
	if (timeoutOccurred())
		return 0xffff;
	else
