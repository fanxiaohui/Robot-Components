...
{
	last_status = i2c_transmit(address, &reg, 1);
	last_status = i2c_transmit(address, &value, 2);
}
...
{
	last_status = i2c_transmit(address, &reg, 1);
	last_status = i2c_transmit(address, &value, 4);
}
...
u16 readReg16Bit(u8 reg)
{
	...
	last_status = i2c_transmit(address, &reg, 1);
	last_status = i2c_receive(address, &value, 2);
...
u32 readReg32Bit(u8 reg)
{
	...
	last_status = i2c_transmit(address, &reg, 1);
	last_status = i2c_receive(address, &value, 4);
